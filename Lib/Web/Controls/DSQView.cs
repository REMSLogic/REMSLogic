...
namespace Lib.Web.Controls
{
	...
	public class DSQView : WebControl
	{
		...
		{
            _userProfile = Security.GetCurrentProfile();
			...
			if( has_children || eoc_applies )
			{
                ...
                */
				...
				if( eoc_applies && has_drug )
				{
                    ...
                    PrescriberEoc prescriberEoc = eocs.FirstOrDefault();
					...
					writer.RenderBeginTag( "div" );
					{
						...
						writer.RenderBeginTag( "div" );
						{
							...
							else
							{
								...
								writer.AddAttribute( "href", "/api/App/Users/Certified?drug_id=" + Drug.ID.Value + "&question_id=" + q.ID );
