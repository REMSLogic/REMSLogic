...
(function ($) {
	$.drillDownMenu = function (el, options) {
		...
		base.init = function () {
			...
			$(".main-menu", base.$el).bind("click.drillDownMenu", $.proxy(function (event) {
				var currUl = base.$el.data("currentMenu");
				if (currUl.length) {
					currUl.trigger("gohome");
				}
				event.preventDefault();
			},
            base.$el)
		...
		base._initUl = function (ul) {
			...
			$("> li:has(ul)", ul).each(function () {
				...
				currUl.bind("goback.drillDownMenu", function (event) {
					...
					base.$el.css('height', ((prevUl.height()) + ($('h1', base.$el).height() + 2)).toString() + 'px');
				...
				currUl.bind("gohome.drillDownMenu", function (event) {
					var currUl = $(this);
					var prevUl = $(this).parent().closest("ul");
					var parentUl = $(this).parent().closest("ul");
					var count = 0;
					while (parentUl.length) {
						prevUl = parentUl;
						parentUl = prevUl.parent().closest("ul");
						count++;
					}

					if (prevUl.length) {
						tlm.stop(false, true).animate({ left: "+="+count.toString()+"00%" }, 300, "easeInOutQuart", function () {
							currUl.hide();
						});
						base.$el.data("currentMenu", prevUl);
						prevUl.hasClass("tlm") && (toolbar.hide(), searchform.show());
				...
				menuItem.bind("click.drillDownMenu", $.proxy(function (event) {
					...
					nextUl.show();
					base.$el.css('height', ((nextUl.height()) + ($('h1', base.$el).height()+2)).toString() + 'px');
