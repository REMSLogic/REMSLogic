...
<div class="container_12 clearfix leading">
    <div class="grid_12 facility-add-wrap">
        ...
        <form class="has-validation ajax-form" style="margin-bottom: 32px;" action="/api/Admin/Security/Provider/Edit?id=<%=((Organization.Id == null) ? 0 : Organization.Id)%>">
            <h2>Oganization Details</h2>
            <div class="form">
                <div class="clearfix">
            ...
            <div class="form" style="<%=(Organization.Id > 0)? "display: none;" : ""%>">
                ...
                <div class="clearfix">
                    <label for="form-facility-size" class="form-label">Size <em>*</em></label>
		...
		<% if(Organization.Id > 0 ) { %>
		<a href="#hcos/facilities/edit?id=0&provider-id=<%=Organization.Id%>" class="button" style="float: right; margin-top: 46px; margin-bottom: 10px;">Add Facility</a>
		<h2 style="margin-top: 36px;">Facilities</h2>
		<div class="clearfix">
            <table id="facilities-table" class="display provider-edit-dt">
				...
				<% foreach( var facility in Organization.Facilities ) {	%>
                    <tr data-id="<%=facility.Id%>">
						<td>
						    <a href="#hcos/facilities/edit?id=<%=facility.Id%>&provider-id=<%=Organization.Id%>" class="button">Edit</a> 
                            <%if(facility.Id != Organization.PrimaryFacilityId){ %>
                            <a href="/api/HCOs/Facility/Delete?id=<%=facility.Id%>" class="ajax-button button" data-confirmtext="Are you sure you want to delete this facility?">Delete</a>
                            <%} %>
						</td>
						<td><%=facility.Name%></td>
                        <td><%=facility.Id == Organization.PrimaryFacilityId? "Yes" : String.Empty%></td>
		...
		<div class="clearfix">
            <table class="display provider-edit-dt">
                ...
                <tbody>
				<% foreach( var profile in Prescribers ) { %>
        ...
        <div>
            <table class="display provider-edit-dt">
                ...
                <% foreach(var profile in PendingInvites) { %>
                    <tr data-id="<%=profile.ID%>">
                        <td><a target="_blank" href="/SignUp.aspx#prescriber/wizards/landing-page?token=<%=profile.Guid%>" class="button">Enroll</a></td>
                        <td><%=profile.Contact.LastName + ", " + profile.Contact.FirstName%></td>
                        <td><%=profile.Contact.Email%></td>
                    </tr>
                <% } %>
