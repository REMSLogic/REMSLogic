<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="etasu-selections.ascx.cs" Inherits="Site.Views.ecommerce.wizards.etasu_selections" %>
...
<div class="container_12 clearfix leading">  
    <div class="grid_12">
        ...
        <ul style="margin-bottom: 32px; font-size: 16px;">
        ...
        <form class="form ajax-form" action="/api/Ecommerce/Wizards/ETASUSelections">
            <%foreach(Drug drug in Drugs){ %>
            <div class="clearfix">
                <label for="form-drug-selections-<%=drug.ID%>" class="form-label" style="width: 75%;"><%=drug.GenericName%></label>
                <div class="form-input" style="width: 25%"><div class="radiogroup"><label><input type="radio" name="drug-selection[<%=drug.ID%>]" id="form-drug-selections-<%=drug.ID%>" required="required" value="Yes" /> Yes</label> <label><input type="radio" name="drug-selection[<%=drug.ID%>]" value="No" checked="checked" /> No</label></div></div>
            </div>
            <%}%>
